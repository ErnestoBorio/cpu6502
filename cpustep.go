...
func (cpu *CPU) Step() uint8 {
	...
	switch opcode {
		...
		// STA STX STY
		case STY_Zp_84: cpu.storeReg(cpu.Y, cpu.zeroPage())
		case STA_Zp_85: cpu.storeReg(cpu.A, cpu.zeroPage())
		case STX_Zp_86: cpu.storeReg(cpu.X, cpu.zeroPage())
		case STY_ZpX_94: cpu.storeReg(cpu.Y, cpu.zeroPageIndexed(cpu.X))
		case STA_ZpX_95: cpu.storeReg(cpu.A, cpu.zeroPageIndexed(cpu.X))
		case STX_ZpY_96: cpu.storeReg(cpu.X, cpu.zeroPageIndexed(cpu.Y))
		case STY_Abs_8C: cpu.storeReg(cpu.Y, cpu.absolute())
		case STA_Abs_8D: cpu.storeReg(cpu.A, cpu.absolute())
		case STX_Abs_8E: cpu.storeReg(cpu.X, cpu.absolute())
		case STA_AbsY_99: cpu.storeReg(cpu.A, cpu.absoluteIndexed(cpu.Y))
		case STA_AbsX_9D: cpu.storeReg(cpu.A, cpu.absoluteIndexed(cpu.X))
		case STA_IndexIndirX_81: cpu.storeReg(cpu.A, cpu.indexedIndirectX())
		case STA_IndirIndexY_91: cpu.storeReg(cpu.A, cpu.indirectIndexedY())
		...
		case INC_Zp_E6: cpu.incDec(cpu.zeroPage(), +1 )
		case DEC_Zp_C6: cpu.incDec(cpu.zeroPage(), -1 )
		case INC_ZpX_F6: cpu.incDec(cpu.zeroPageIndexed(cpu.X), +1 )
		case DEC_ZpX_D6: cpu.incDec(cpu.zeroPageIndexed(cpu.X), -1 )
		case INC_Abs_EE: cpu.incDec(cpu.absolute(), +1 )
		case DEC_Abs_CE: cpu.incDec(cpu.absolute(), -1 )
		case INC_AbsX_FE: cpu.incDec(cpu.absoluteIndexed(cpu.X), +1 )
		case DEC_AbsX_DE: cpu.incDec(cpu.absoluteIndexed(cpu.X), -1 )
		// ADC SBC
		...
		case ASL_Zp_06:   cpu.asl(cpu.zeroPage())
		case LSR_Zp_46:   cpu.lsr(cpu.zeroPage())
		case ROL_Zp_26:   cpu.rol(cpu.zeroPage())
		case ROR_Zp_66:   cpu.ror(cpu.zeroPage())
		case ASL_ZpX_16:  cpu.asl(cpu.zeroPageIndexed(cpu.X))
		case LSR_ZpX_56:  cpu.lsr(cpu.zeroPageIndexed(cpu.X))
		case ROL_ZpX_36:  cpu.rol(cpu.zeroPageIndexed(cpu.X))
		case ROR_ZpX_76:  cpu.ror(cpu.zeroPageIndexed(cpu.X))
		case ASL_Abs_0E:  cpu.asl(cpu.absolute())
		case LSR_Abs_4E:  cpu.lsr(cpu.absolute())
		case ROL_Abs_2E:  cpu.rol(cpu.absolute())
		case ROR_Abs_6E:  cpu.ror(cpu.absolute())
		case ASL_AbsX_1E: cpu.asl(cpu.absoluteIndexed(cpu.X))
		case LSR_AbsX_5E: cpu.lsr(cpu.absoluteIndexed(cpu.X))
		case ROL_AbsX_3E: cpu.rol(cpu.absoluteIndexed(cpu.X))
		case ROR_AbsX_7E: cpu.ror(cpu.absoluteIndexed(cpu.X))
